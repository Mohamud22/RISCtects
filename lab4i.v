...
`define SIZE_WORD  2'b10
   ...
   module PipelinedCPUCPU(halt, clk, rst);
...
/*-------------------------------------------pipeline registers --------------------------------------------------------------*/
 // Intermediate pipeline registers
   ...
   reg [`WORD_WIDTH-1:0] EX_MEM_ALUOut, EX_MEM_Rdata2, EX_MEM_PC;
   reg [4:0] EX_MEM_Rdst;
...
/*----------------------------------Start of Instruction fetch ---------------------------*/
   ...
   assign PC = PC_REG;
   assign NPC = (Branch || Jump) ? ID_EX_PC + ID_EX_Immediate : PC + 4;

   // System State 
   Mem   MEM(.InstAddr(PC), .InstOut(InstWord), 
            .DataAddr(DataAddr), .DataSize(MemSize), .DataIn(StoreData), .DataOut(DataWord), .WE(MemWrEn), .CLK(clk));
   
   // always chunk block I wrote
   always @(posedge clk) begin
      IF_ID_PC <= PC;
      IF_ID_InstWord <= InstWord;
   end
  // Forwarding Control Logic
   assign forwardA_sel = (EX_MEM_Rdst == ID_EX_Rsrc1 && EX_MEM_opcode != `OPCODE_STORE) ? 2'b10 :
                         (MEM_WB_Rdst == ID_EX_Rsrc1 && MEM_WB_opcode != `OPCODE_STORE) ? 2'b01 : 2'b00;

   assign forwardB_sel = (EX_MEM_Rdst == ID_EX_Rsrc2 && EX_MEM_opcode != `OPCODE_STORE) ? 2'b10 :
                         (MEM_WB_Rdst == ID_EX_Rsrc2 && MEM_WB_opcode != `OPCODE_STORE) ? 2'b01 : 2'b00;

   // Forwarded ALU inputs
   assign alu_inputA = (forwardA_sel == 2'b10) ? EX_MEM_ALUOut :
                       (forwardA_sel == 2'b01) ? DRin : ID_EX_Rdata1;

   assign alu_inputB = (forwardB_sel == 2'b10) ? EX_MEM_ALUOut :
                       (forwardB_sel == 2'b01) ? DRin : ID_EX_Rdata2;
...
/*----------------------------------end of instruction fetch ---------------------------*/
   ...
   /*----------------------------------Memory and WB start here ---------------------------*/
//  Memory Access (MEM)
   assign MemWrEn = (EX_MEM_opcode == `OPCODE_STORE);
   assign DataAddr = EX_MEM_ALUOut;
   assign StoreData = EX_MEM_Rdata2;

   always @(posedge clk) begin
      MEM_WB_MemData <= DataWord;
   ...
   // Write Back (WB)
   assign DRin = (MEM_WB_opcode == `OPCODE_LOAD) ? MEM_WB_MemData : MEM_WB_ALUOut;
   assign RWrEn = (MEM_WB_opcode == `OPCODE_COMPUTE_IMM || 
                   MEM_WB_opcode == `OPCODE_COMPUTE || 
                   MEM_WB_opcode == `OPCODE_LOAD);

   // Halt logic
   assign halt = invalid_case | bad_mem_alignment;
/*----------------------------------Memory and WB end here ---------------------------*/
   // Instruction Decode
...
// Execute instruction to be included in the ALU- maybe add proper delay and cycles
...
/* -----------------------Start Execute Instruction forwarding -----------------------------*/
// Execute (EX) instruction 
   ExecutionUnit EU(.out(out), 
                    .opA(alu_inputA), 
                    .opB(ID_EX_opcode == `OPCODE_COMPUTE_IMM ? ID_EX_Immediate : alu_inputB), 
                    .func(ID_EX_funct3), 
                    .auxFunc(ID_EX_funct7), 
                    .opcode(ID_EX_opcode), 
                    .immediate(ID_EX_Immediate));

   assign Branch = (ID_EX_opcode == `OPCODE_BRANCH) && 
                   ((ID_EX_funct3 == `FUNC_BEQ && alu_inputA == alu_inputB) ||
                    (ID_EX_funct3 == `FUNC_BNE && alu_inputA != alu_inputB));
...
/* ------------------------End Execute Instruction forwarding --------------------------------*/
    // for computes
